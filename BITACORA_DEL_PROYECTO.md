...
|---|---|---|---|
| **17/08 (Codex, en curso)** | Estructura GitHub del workspace | Se prepara una migración aislada de Onegai-WorkSpace para que el repositorio reúna canon, cartas, motor y puente. La copia se hace fuera de Software/: no modifica los repositorios locales ni copia sus metadatos .git. Se excluyen builds, caches, entornos, papelera y recursos de terceros ya ignorados por el canon (incluidos el ZIP de 830 MB y PDF de 141 MB). Pendiente: verificar y publicar la rama codex/workspace-structure. | pendiente |
