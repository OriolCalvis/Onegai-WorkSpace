  ...
  recommend(input: {
    ...
    if (/(literary|experimental|memoir|essay|mosaic|fragment)/i.test(genreLower + ' ' + subgenreLower + ' ' + descLower)) {
      ...
      additionalNotes.push(
        ...
        `Many literary novels deliberately break structure for effect. AuthorAgent won't force a beat sheet you didn't pick.`
      );
    }
  ...
  checkOutline(outline: string[], structureId: StructureId): OutlineCheckReport | null {
    ...
    if (structure.id === 'none' || structure.beats.length === 0) {
      return {
        ...
        summary: `No structure selected — beat checking skipped. AuthorAgent won't enforce structure on this project.`,
